...
if [ "$DOMODELS" == true ]; then
  echo "-- Doing models --"
  touch makingModels
  CUDA_VISIBLE_DEVICES=$GPUINDEX $SINGULARITYCOMAND colabfold_batch --model-type ${MODELTYPE} $MINIMISATION --num-seeds $REPEAT --num-recycle $NUMRECYCLE /inout/msas /inout/predictions
  rm makingModels
